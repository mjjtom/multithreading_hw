...
// 1 отправитель, 1 получатель, базовая проверка
TEST(Correctness, Simple) {
    RunTest(1, 1, 1, 300);
}

// много отправителей, проверка записи из нескольких потоков
TEST(Correctness, Senders) {
    RunTest(4, 1, 3, 300);
}

// много получателей, проверка чтения из нескольких потоков
TEST(Correctness, Receivers) {
    RunTest(1, 6, 4, 300);
}

// маленький буфер, частые блокировки
TEST(Correctness, SmallBuf) {
    RunTest(8, 2, 2, 300);
}

// большой буфер, почти без блокировок
TEST(Correctness, BigBuf) {
    RunTest(3, 3, 100, 300);
}

// стресс-тест, много потоков с обеих сторон
